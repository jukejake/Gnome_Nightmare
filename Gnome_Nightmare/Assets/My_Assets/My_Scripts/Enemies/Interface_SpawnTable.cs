...
public class Interface_SpawnTable : MonoBehaviour {
    ...
    private Spawner_Manager[] spawnerManager;
    private Spawner_Settings[] spawnerSettings;
    ...
    private int CheckTotalEnemyCount() {
        ...
        for (int i = 0; i < spawnerManager.Length; i++) {
            for (int j = 0; j < spawnerManager[i].enemySpawnTable.spawners.Count; j++) {
                ...
                if (ToggleAll || spawnerManager[i].enemySpawnTable.spawners[i].Toggle || spawnerManager[i].enemySpawnTable.spawners[i].SpawnerPosition == null) { }
                //enemySpawnTable.spawners[i].LastAvtiveRound will be set as the current round... (only for this)
                else if ((CurrentLevel >= spawnerManager[i].enemySpawnTable.spawners[i].StartAt && CurrentLevel == spawnerManager[i].enemySpawnTable.spawners[i].LastAvtiveRound) || (CurrentLevel == spawnerManager[i].enemySpawnTable.spawners[i].StartAt)) {
                    //counts amount of enemies to spawn multiplyed by number of waves in spawner
                    EnemyCount += (spawnerManager[i].enemySpawnTable.spawners[i].spawnersDetails.TotalEnemyCount() * spawnerManager[i].enemySpawnTable.spawners[i].spawnersDetails.NumberOfWaves);
                }
                spawnerManager[i].enemySpawnTable.spawners[i].spawnersDetails.DefeatedSpawner = true;
            }
        }
    ...
    public void ActivateAllSpawnersInCurrentRound() {
        ...
        for (int i = 0; i < spawnerManager.Length; i++) {
            for (int j = 0; j < spawnerManager[i].enemySpawnTable.spawners.Count; j++) {
                //If spawner is not active or it does not have the component than return
                if (spawnerManager[i].enemySpawnTable.spawners[i].Toggle) { }
                else if ((CurrentLevel >= spawnerManager[i].enemySpawnTable.spawners[i].StartAt && CurrentLevel == (spawnerManager[i].enemySpawnTable.spawners[i].LastAvtiveRound + spawnerManager[i].enemySpawnTable.spawners[i].ActiveEvery)) || (CurrentLevel == spawnerManager[i].enemySpawnTable.spawners[i].StartAt)) {
                    spawnerManager[i].enemySpawnTable.spawners[i].LastAvtiveRound = CurrentLevel;
                    //If the spawner is deactive then active it
                    if (spawnerManager[i].enemySpawnTable.spawners[i].spawnersDetails.DefeatedSpawner) {
                        spawnerManager[i].enemySpawnTable.spawners[i].spawnersDetails.DefeatedSpawner = false;
                        spawnerManager[i].enemySpawnTable.spawners[i].spawnersDetails.Round = TimeBetweenRounds;
                        spawnerManager[i].enemySpawnTable.spawners[i].spawnersDetails.spawnCoolDownRemaining = TimeBetweenRounds;
                        spawnerManager[i].enemySpawnTable.spawners[i].spawnersDetails.IncreaseStats();
                    }
                }
            }
        }
    ...
    public void SetAllIntervalBetweenSpawns(float amount) {
        ...
        for (int i = 0; i < spawnerManager.Length; i++) {
            for (int j = 0; j < spawnerManager[i].enemySpawnTable.spawners.Count; j++) {
                spawnerManager[i].enemySpawnTable.spawners[i].spawnersDetails.Spawn = amount;
            }
        }
        for (int i = 0; i < spawnerSettings.Length; i++) {
    ...
    public void SetAllIntervalBetweenRounds(float amount) {
        ...
        for (int i = 0; i < spawnerManager.Length; i++) {
            for (int j = 0; j < spawnerManager[i].enemySpawnTable.spawners.Count; j++) {
                spawnerManager[i].enemySpawnTable.spawners[i].spawnersDetails.Round = amount;
            }
        }
        for (int i = 0; i < spawnerSettings.Length; i++) {
